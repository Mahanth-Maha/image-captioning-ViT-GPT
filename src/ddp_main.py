...
def main():
    ...
    try:
        ...
        with torch.no_grad():
            ...
            output_logits = test_model(images, captions[:, :-1])
            labels = captions[:, 1:]
            ...
            del test_model, images, captions
    ...
    if is_main_process and args.overfit:
        ...
        training_config['avg_overfit_time'] = avg_overfit_time.item()
        if is_main_process:
    ...
    if is_main_process and training_config['train_time_warmup'] > 0:
        warmup_model = copy.deepcopy(model)
        # optimizer = create_optimizer(model, lr=float(training_config.get('learning_rate', 1e-4)))
        optimizer = torch.optim.Adam(warmup_model.parameters(), lr=2e-4)
        ...
        warmup_trainer_config = copy.deepcopy(trainer_config)
        warmup_trainer_config['use_tensorboard'] = False
        warmup_trainer_config['use_wandb'] = False
        warmup_log_dir = training_config['log_dir'] + '/warmup'
        ...
        # os.rmdir(warmup_log_dir)
        try:
            shutil.rmtree(warmup_log_dir)
        except FileNotFoundError:
            print(f"‼️ Directory '{warmup_log_dir}' not found !?")
        except OSError as e:
            print(f"‼️ Error deleting directory: {e}")
        del warmup_model
        del warmup_trainer
        del warmup_trainer_config
        torch.cuda.empty_cache()
        gc.collect()
    
    if args.train:
        if is_main_process:
            console.print("[yellow]🖋️  Starting Training...[/yellow]")
